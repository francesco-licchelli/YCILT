...
fun LoginScreen(
	...
	Column(
		...
		Button(
			onClick = { onLoginClick(usernameState.value, passwordState.value) },
			modifier = Modifier
				.fillMaxWidth()
				.padding(top = 16.dp),
		...
		Button(
			...
			Text(
				text = stringResource(id = R.string.don_t_have_an_account_signup),
				color = Color.White
			)
		}

		Button(
			onClick = onDisplayAudioClick,
			modifier = Modifier
				.fillMaxWidth()
				.padding(top = 16.dp),
			colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.primary)
			...
			Text(
				text = stringResource(id = R.string.my_audio),
				color = Color.White
			)
		}
	}
